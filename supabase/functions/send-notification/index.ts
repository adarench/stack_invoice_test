...
const SUBJECT_MAP: Record<string, (inv: any) => string> = {
  review_pending: () => 'Invoice Pending Approval — OpsFlow',
...
const ACTION_DESCRIPTION: Record<string, string> = {
  review_pending: 'is pending your approval',
...
function buildHtml(action: string, invoice: any, actor: any, link?: string): string {
  ...
  <div style="max-width: 480px; margin: 0 auto; background: white; border-radius: 12px; border: 1px solid #e2e8f0; overflow: hidden;">
    ...
    <div style="padding: 24px;">
      <p style="margin: 0 0 16px; font-size: 14px; color: #334155; line-height: 1.5;">
        You have an invoice pending review.
      </p>
      <table style="width: 100%; font-size: 13px; color: #475569; border-collapse: collapse;">
        <tr><td style="padding: 6px 0; color: #94a3b8;">Invoice</td><td style="padding: 6px 0; text-align: right; font-weight: 600; color: #0f172a;">#${invNum}</td></tr>
        ...
        ${invoice.property_name ? `<tr><td style="padding: 6px 0; color: #94a3b8;">Property</td><td style="padding: 6px 0; text-align: right; font-weight: 500;">${invoice.property_name}</td></tr>` : ''}
      </table>
      ${link ? `
      <div style="margin-top: 20px;">
        <a href="${link}" style="display: inline-block; padding: 10px 14px; border-radius: 8px; background: #1d4ed8; color: white; text-decoration: none; font-size: 13px; font-weight: 600;">
          Review invoice
        </a>
      </div>` : ''}
    </div>
...
serve(async (req) => {
  ...
  try {
    ...
    const { action, invoice, recipients, actor, link } = await req.json()
    ...
    const html = buildHtml(action, invoice || {}, actor, link)
